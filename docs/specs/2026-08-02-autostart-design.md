...
> ## 🛑 本方案已于 2026-08-02 放弃，不会实现
>
> **放弃原因：复杂度与收益不成比例。** 用户的硬约束（「不开启就不碰我电脑」+「随时可
> 取消恢复」）叠加四个平台各不相同的启动机制，把「加个开关」推成了一套需要归属标记、
> 冲突识别、跨进程锁和逐步补偿的系统——经五轮评审才收敛到可开工，而它解决的问题
> （开机/登录自启）本来就不是用户想要的。
>
> **改为实现的东西**：跟随 VSCode 启动的按需拉起（本来就在用的机制）+ 一个开关，
> 其他系统环境走「建快捷方式手动打开」。见 README 的〈跟随编辑器启动〉一节。
>
> **本文存档的价值**：里面记录了若干经查证的事实，日后若真要做自启动可直接复用——
> `systemctl enable/disable` 默认隐式 reload、`StartLimitIntervalSec` 属于 `[Unit]`、
> XDG autostart 不监督进程、macOS 13+ 的登录项授权状态无法用 `launchctl print` 判定、
> `.desktop` 与 `.lnk` 都没有环境变量槽位。
>
> 唯一落地的产物是 `server/launch.py` 的 `--config` 参数（提交 `fc51169`），它独立可用，
> 已保留。

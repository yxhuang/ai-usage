...
从本机已有的登录凭证里读出三家的订阅额度，一家一张卡片：每个用量窗口
（5 小时 / 周 / 额外 credit）画成一根进度条，条上带一道刻度，标出这个窗口已经
过去了多少。后台定时轮询，只绑本机回环地址，不写你的凭证文件。
...
开关在面板里：点标题左边的齿轮进设置，第一项就是「跟随 VSCode 启动」。钩子没装的时候
它会直说「未检测到编辑器钩子」，不会给你一个点了没用的开关。

...
<p align="center">
  <img src="panel-dark.png" width="380"
       alt="深色模式下的 ai-usage 面板：三张卡片各对应一家。Claude Pro 的 5 小时窗口 67%、周额度 22%、额外用量 credit 23%（$22.98 / $100）；Codex Plus 周额度 12%；Kimi 的 5 小时窗口 3%、周额度 2%。每根进度条上都有一道发光竖线，标出时间窗口已经过去的比例：Claude 的 5 小时条还没够到刻度，周额度条则已经越过。">
</p>

看上图里的 Claude：5 小时窗口那根条还没够到刻度，说明还能接着用；周额度那根却已经越过了
自己的刻度，说明这笔预算花超前了。同一个账号、同一时刻，两个相反的结论——而光看百分比
（67% 和 22%）你得到的判断恰好是反的。那道竖线才是重点。

## 节奏刻度

**这就是这个面板的功能本体**，仓库里其余部分都是为了把它画出来而存在的管道。

那道发着光的竖线标出的是**时间窗口已经过去了多少**。用量条要拿它当尺子来读：
...
落在刻度左边，说明你花得比额度回补更慢，可以继续用；越过它，说明照这个速度会在重置前
用完，而且越过越多、撞墙越早。光看百分比只能知道「我用了多少」，节奏刻度回答的是
「我还能不能这么用」——周三下午两点你真正想知道的是后面这个问题。

它被画成一根两端探出条外的发光指针，还带一圈描边，压在品牌色填充上也不会糊掉。
这是整个克制的面板里唯一张扬的一处，而且是刻意的：**要眯着眼才能看清的刻度，等于没有**。
它仍用墨色而不用红色——红在这里已经专指「用量 ≥90%」，再来一个含义不同的红，
两个信号都会变钝。
